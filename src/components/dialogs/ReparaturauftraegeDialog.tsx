...
import type { Reparaturauftraege, Kundenverwaltung, LookupValue } from '@/types/app';
...
export function ReparaturauftraegeDialog({ open, onClose, onSubmit, defaultValues, recordId, kundenverwaltungList, enablePhotoScan = true, enablePhotoLocation = true }: ReparaturauftraegeDialogProps) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "auftragsnummer": string | null, // Auftragsnummer\n  "eingangsdatum": string | null, // YYYY-MM-DD\n  "kunde": string | null, // Display name from Kundenverwaltung (see <available-records>)\n  "fahrzeug_bezeichnung": string | null, // Fahrzeug / Gerät\n  "kennzeichen": string | null, // Kennzeichen / Seriennummer\n  "problembeschreibung": string | null, // Problembeschreibung\n  "mitarbeiter": string | null, // Zuständiger Mitarbeiter\n  "voraussichtliche_fertigstellung": string | null, // YYYY-MM-DD\n  "status": LookupValue | null, // Status (select one key: "eingegangen" | "in_bearbeitung" | "fertig_zur_abholung") mapping: eingegangen=Eingegangen, in_bearbeitung=In Bearbeitung, fertig_zur_abholung=Fertig zur Abholung\n  "interne_notizen": string | null, // Interne Notizen\n}`;
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'fahrzeug_bezeichnung': (
      <div key="fahrzeug_bezeichnung" className="space-y-1.5">
        <Label htmlFor="fahrzeug_bezeichnung">Fahrzeug / Gerät</Label>
        <Input
          id="fahrzeug_bezeichnung"
          placeholder="z. B. BMW 3er, Bohrmaschine"
    ...
    'kennzeichen': (
      <div key="kennzeichen" className="space-y-1.5">
        ...
        <Input
          id="kennzeichen"
          placeholder="z. B. B-AB 1234, SN123456"
    ...
    'problembeschreibung': (
      <div key="problembeschreibung" className="space-y-1.5">
        ...
        <Textarea
          ...
          placeholder="Beschreibe das Problem — was funktioniert nicht, welche Fehler?"
    ...
    'voraussichtliche_fertigstellung': (
      <div key="voraussichtliche_fertigstellung" className="space-y-1.5">
        <Label htmlFor="voraussichtliche_fertigstellung">Voraussichtliche Fertigstellung</Label>
        <DatePicker
          id="voraussichtliche_fertigstellung"
          placeholder="Wann ist es voraussichtlich fertig?"
          mode="date"
    ...
    'interne_notizen': (
      <div key="interne_notizen" className="space-y-1.5">
        ...
        <Textarea
          id="interne_notizen"
          placeholder="Besonderheiten, Ersatzteil-Infos, Fehlersuche-Schritte..."
  ...
  const FIELD_LABELS: Record<string, string> = {"auftragsnummer": "Auftragsnummer", "eingangsdatum": "Eingangsdatum", "kunde": "Kunde", "fahrzeug_bezeichnung": "Fahrzeug / Gerät", "kennzeichen": "Kennzeichen / Seriennummer", "problembeschreibung": "Problembeschreibung", "mitarbeiter": "Zuständiger Mitarbeiter", "voraussichtliche_fertigstellung": "Voraussichtliche Fertigstellung", "status": "Status", "interne_notizen": "Interne Notizen"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"kunde": {"vorname": "Vorname", "nachname": "Nachname", "telefon": "Telefonnummer", "email": "E-Mail-Adresse", "notiz_kunde": "Notizen zum Kunden"}};
