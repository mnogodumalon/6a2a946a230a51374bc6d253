...
export function ReparaturauftraegeViewDialog({ open, onClose, record, onEdit, kundenverwaltungList }: ReparaturauftraegeViewDialogProps) {
    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">Fahrzeug / Gerät</Label>
            <p className="text-sm">{record.fields.fahrzeug_bezeichnung ?? '—'}</p>
          </div>
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">Kennzeichen / Seriennummer</Label>
            <p className="text-sm">{record.fields.kennzeichen ?? '—'}</p>
          </div>
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">Problembeschreibung</Label>
          ...
          <div className="space-y-1">
            ...
            <p className="text-sm">{record.fields.mitarbeiter ?? '—'}</p>
          </div>
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">Voraussichtliche Fertigstellung</Label>
            <p className="text-sm">{formatDate(record.fields.voraussichtliche_fertigstellung)}</p>
          </div>
          <div className="space-y-1">
