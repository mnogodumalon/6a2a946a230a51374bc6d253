...
export function KundenverwaltungDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: KundenverwaltungDialogProps) {
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'notiz_kunde': (
      <div key="notiz_kunde" className="space-y-1.5">
        <Label htmlFor="notiz_kunde">Notizen zum Kunden</Label>
        <Textarea
          id="notiz_kunde"
          placeholder="Zahlungsweise, Vorlieben, Besonderheiten..."
          value={fields.notiz_kunde ?? ''}
          onChange={e => setFields(f => ({ ...f, notiz_kunde: e.target.value }))}
          rows={3}
        />
      </div>
    ),
  ...
  const FIELD_LABELS: Record<string, string> = {"vorname": "Vorname", "nachname": "Nachname", "telefon": "Telefonnummer", "email": "E-Mail-Adresse", "notiz_kunde": "Notizen zum Kunden"};
