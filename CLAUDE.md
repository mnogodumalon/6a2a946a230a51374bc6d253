...
- Edit `src/App.tsx` to add imports and routes for the new intent pages. **CRITICAL:** Place imports ONLY inside the `// <custom:imports>` / `// </custom:imports>` marker block, and routes ONLY inside the `{/* <custom:routes> */}` / `{/* </custom:routes> */}` marker block. Everything outside those markers is scaffold and will be overwritten on the next `/build/update` — your additions would be lost. Example:
  ...
  {/* </custom:routes> */}
  ```
- Edit `src/config/intents.ts` to REGISTER every intent page — that puts it into the SIDEBAR automatically (the `IntentsNav` section renders from this registry; no Layout edit). Same marker discipline: icon imports go inside `// <custom:intent-imports>`, entries inside `// <custom:intents>`. `path` must equal the route from App.tsx; `label` is 1–3 German words; pick a fitting Tabler icon (COMPONENT reference, not JSX). Example:
  ```ts
...
- **`src/config/intents.ts`** — The orchestrator MUST register every intent page here (inside the `<custom:intents>` markers) — this is what makes them appear in the sidebar. Entries outside the markers are lost on the next scaffold update.
